...
function reusePreCompletedPhase2Seed(parser, state, preCompletedResult, exec) {
	...
	return result.success ? require_phase2_seed.phase2SeedFromValueResult(result) : require_phase2_seed.extractPhase2Seed(parser, state, exec);
}
async function reusePreCompletedPhase2SeedAsync(parser, state, preCompletedResult, exec) {
	const result = unwrapCompleteResult(preCompletedResult);
	return result.success ? require_phase2_seed.phase2SeedFromValueResult(result) : await require_phase2_seed.extractPhase2Seed(parser, state, exec);
}
function extractOrCompletePhase2SeedSync(parser, state, exec) {
	const extracted = require_phase2_seed.extractPhase2Seed(parser, state, exec);
	if (extracted != null) return extracted;
	const result = parser.complete(state, exec);
	return result.success ? require_phase2_seed.phase2SeedFromValueResult(result) : null;
}
async function extractOrCompletePhase2SeedAsync(parser, state, exec) {
	const extracted = await require_phase2_seed.extractPhase2Seed(parser, state, exec);
	if (extracted != null) return extracted;
		...
		[require_phase2_seed.extractPhase2SeedKey](state, exec) {
			const extractMergeCompleteState = (parser, resolvedState, index) => {
				if (parser.initialState === void 0) {
					...
					if (resolvedState && typeof resolvedState === "object") {
						const extractedState = {};
						for (const field in parser.initialState) extractedState[field] = field in resolvedState ? resolvedState[field] : parser.initialState[field];
						return require_annotations.inheritAnnotations(resolvedState, extractedState);
					}
					return parser.initialState;
				}
				return parser.initialState;
			...
			return require_mode_dispatch.dispatchByMode(combinedMode, () => {
				...
				for (let i = 0; i < syncParsers.length; i++) {
					...
					const seed = require_phase2_seed.completeOrExtractPhase2Seed(parser, parserState, completeExec);
					if (seed == null) continue;
					hasAnySeed = true;
				...
				for (let i = 0; i < parsers.length; i++) {
					...
					const seed = await require_phase2_seed.completeOrExtractPhase2Seed(parser, parserState, completeExec);
					if (seed == null) continue;
					hasAnySeed = true;
					const seedValue = seed.value;
					for (const field in seedValue) {
...
//#endregion
exports.DuplicateOptionError = DuplicateOptionError;
